...
function of_style_only(){
	wp_enqueue_style('olam-options-admin-style', ADMIN_DIR . 'assets/css/admin-style.css');
	wp_enqueue_style('jquery-ui-custom-admin', ADMIN_DIR .'assets/css/jquery-ui-custom.css');
	if ( !wp_style_is( 'wp-color-picker','registered' ) ) {
		wp_enqueue_style( 'wp-color-picker', ADMIN_DIR . 'assets/css/color-picker.min.css' );
	}else{
...
function of_load_only() 
{
	...
	if ( !wp_script_is( 'wp-color-picker', 'registered' ) ) {
		wp_enqueue_script( 'iris', ADMIN_DIR .'assets/js/iris.min.js', array( 'jquery-ui-draggable', 'jquery-ui-slider', 'jquery-touch-punch' ), false, 1 );
		wp_enqueue_script( 'wp-color-picker', ADMIN_DIR .'assets/js/color-picker.min.js', array( 'jquery', 'iris' ) );
	}
	else{
...
function of_ajax_callback() 
{
	...
	if($save_type == 'upload')
	{
		...
		$uploaded_file = wp_handle_upload($filename,$override);
		 ...
		 if(!empty($uploaded_file['error'])) {esc_html__e('Upload Error:','olam') . $uploaded_file['error']; }	
