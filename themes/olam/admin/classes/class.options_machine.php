...
class Options_Machine {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'slider':
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button slide_add_button">'.esc_html__("Add New Slide","olam").'</a></div>';
				...
				case 'backup':
					...
					if(!isset($backup['backup_log'])) {
						$log = esc_html__('No backups yet','olam');
					} else {
					...
					$output .= '<p><strong>'. esc_html__('Last Backup : ','olam').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
					$output .= '<a href="#" id="of_backup_button" class="button" title="'.esc_attr(esc_html__("Backup Options","olam")).'">'.esc_html__("Backup Options","olam").'</a>';
					$output .= '<a href="#" id="of_restore_button" class="button" title="'.esc_attr(esc_html__("Restore Options","olam")).'">'.esc_html__("Restore Options","olam").'</a>';
					$output .= '</div>';
				...
				case 'transfer':
					...
					$output .= '<a href="#" id="of_import_button" class="button" title="'.esc_attr(esc_html__("Restore Options","olam")).'">'.esc_html__("Import Options","olam").'</a>';
	...
	public static function optionsframework_media_uploader_function($id,$std,$mod){
		...
		if ( function_exists( 'wp_enqueue_media' ) ) {
			$uploader .= '<span class="button media_upload_button" id="'.$id.'">'.esc_html__("Upload","olam").'</span>';
			
			if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
			$uploader .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'" title="' . $id . '">'.esc_html__("Remove","olam").'</span>';
		}
		else 
		{
			$output .= '<p class="upload-notice"><i>'.esc_html__("Upgrade your version of WordPress for full media support.","olam").'</i></p>';
		}
		$uploader .='</div>' . "\n";
		//Preview
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order){
		...
		$slider .= '<a class="slide_edit_button" href="#">'.esc_html__("Edit","olam").'</a></div>';
		
		$slider .= '<div class="slide_body">';
		
		$slider .= '<label>'.esc_html__("Title","olam").'</label>';
		...
		$slider .= '<label>'.esc_html__("Image URL","olam").'</label>';
		$slider .= '<input class="upload slide of-input" name="'. $id .'['.$order.'][url]" id="'. $id .'_'.$order .'_slide_url" value="'. $val['url'] .'" />';
		
		$slider .= '<div class="upload_button_div"><span class="button media_upload_button" id="'.$id.'_'.$order .'">'.esc_html__("Upload","olam").'</span>';
		
		if(!empty($val['url'])) {$hide = '';} else { $hide = 'hide';}
		$slider .= '<span class="button remove-image '. $hide.'" id="reset_'. $id .'_'.$order .'" title="' . $id . '_'.$order .'">'.esc_html__("Remove","olam").'</span>';
		$slider .='</div>' . "\n";
		...
		$slider .= '</div>';	
		$slider .= '<label>'.esc_html__("Link URL (optional)","olam").'</label>';
		$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][link]" id="'. $id .'_'.$order .'_slide_link" value="'. $val['link'] .'" />';
		
		$slider .= '<label>'.esc_html__("Description (optional)","olam").'</label>';
		$slider .= '<textarea class="slide of-input" name="'. $id .'['.$order.'][description]" id="'. $id .'_'.$order .'_slide_description" cols="8" rows="8">'.stripslashes($val['description']).'</textarea>';
	
		$slider .= '<a class="slide_delete_button" href="#">'.esc_html__("Delete","olam").'</a>';
